...
<script>
            ...
            .then(response => {
                if (response.ok || response.redirected) {
                    ...
                    return response.json().then(data => {
                        displayLoginErrors(data.errors || data.message);
                        resetSubmitButton();
                    });
                    ...
                    Swal.fire({
                        ...
                        text: 'You have made too many login attempts. Please try again later.',
                        icon: 'warning',
                    ...
                    return response.json().then(data => {
                        ...
                        if (data.needs_verification || 
                            (data.message && data.message.includes('verify your email')) ||
                            (data.error && data.error.includes('verify your email'))) {
                            
                            // Show email verification error with resend button
                            showEmailVerificationError(data.email || getEmailFromForm());
                        } else {
                            // Handle other login errors
                            let errorMessage = data.message || 'Login failed. Please check your credentials and try again.';
                        ...
                        return response.text().then(text => {
                            ...
                            if (text.includes('invalid credentials') || text.includes('Invalid credentials')) {
                                errorMessage = 'Invalid username/email or password. Please check your credentials.';
                            } else if (text.includes('account') && text.includes('disabled')) {
                                errorMessage = 'Your account has been disabled. Please contact support.';
                            } else if (text.includes('email') && text.includes('verification')) {
                                // Try to extract email and show verification error
    ...
    // Function to display login errors from server
