                            ...
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <div class="d-flex align-items-center">
                                        ...
                                        @php
                                            // Better prize pool calculation for latest draw
                                            $latestCalculatedPrizePool = 0;
                                            
                                            // Priority 1: Use actual prize distribution if available
                                            if ($latestDraw->prize_distribution && is_array($latestDraw->prize_distribution)) {
                                                $latestCalculatedPrizePool = collect($latestDraw->prize_distribution)->sum('amount');
                                            }
                                            // Priority 2: Use stored total_prize_pool
                                            elseif ($latestDraw->total_prize_pool && $latestDraw->total_prize_pool > 0) {
                                                $latestCalculatedPrizePool = $latestDraw->total_prize_pool;
                                            }
                                            // Priority 3: Calculate from tickets sold and settings
                                            else {
                                                $settings = \App\Models\LotterySetting::getSettings();
                                                $latestTotalTickets = $latestDraw->tickets ? $latestDraw->tickets->count() : $latestDraw->total_tickets_sold;
                                                $latestTotalRevenue = $latestTotalTickets * ($settings->ticket_price ?? 2);
                                                $latestAdminCommission = $latestTotalRevenue * (($settings->admin_commission_percentage ?? 10) / 100);
                                                $latestCalculatedPrizePool = $latestTotalRevenue - $latestAdminCommission;
                                            }
                                            
                                            // Ensure minimum display value
                                            $latestCalculatedPrizePool = max($latestCalculatedPrizePool, 0);
                                        @endphp
                                    ...
                                    <div class="card h-100 {{ $draw->status == 'completed' ? 'border-success' : ($draw->status == 'pending' ? 'border-warning' : 'border-secondary') }}">
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="mb-2">
                                                ...
                                                @php
                                                    // Better prize pool calculation logic
                                                    $calculatedPrizePool = 0;
                                                    
                                                    // Priority 1: Use actual prize distribution if available
                                                    if ($draw->prize_distribution && is_array($draw->prize_distribution)) {
                                                        $calculatedPrizePool = collect($draw->prize_distribution)->sum('amount');
                                                    }
                                                    // Priority 2: Use stored total_prize_pool
                                                    elseif ($draw->total_prize_pool && $draw->total_prize_pool > 0) {
                                                        $calculatedPrizePool = $draw->total_prize_pool;
                                                    }
                                                    // Priority 3: Calculate from tickets sold and settings
                                                    else {
                                                        $settings = \App\Models\LotterySetting::getSettings();
                                                        ...
                                                        $calculatedPrizePool = $totalRevenue - $adminCommission;
                                                    }
                                                    
                                                    // Ensure minimum display value
                                                    $calculatedPrizePool = max($calculatedPrizePool, 0);
                                                @endphp
