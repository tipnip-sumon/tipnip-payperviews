...
class PaymentController extends Controller
{
    ...
    /**
     * Show all withdrawals with filtering - Optimized for large datasets (10K-20K records)
     */
    public function withdrawals(Request $request)
    {
        $pageTitle = 'All Withdrawals';
        
        // Set default items per page and allow user selection for large datasets
        $perPage = $request->get('per_page', 100); // Default to 100 for better performance with large data
        $allowedPerPage = [25, 50, 100, 200, 500, 1000, 2000, 5000]; // Support up to 5000 per page for large datasets
        ...
        $withdrawalsQuery = Withdrawal::query()
            ->select([
                'id', 'user_id', 'method_id', 'trx', 'withdraw_type', 'amount', 'charge', 
                'final_amount', 'withdraw_information', 'status', 'created_at'
            ])
            ->with([
                'user:id,username,email', // Only select needed user fields
                'method:id,name' // Include method name for filtering and display
            ])
            ->when($request->search, function($query) use ($request) {
            ...
            ->when($request->method, function($query) use ($request) {
                return $query->where('method_id', $request->method);
            })
        ...
        // Apply ordering optimized for large datasets
        $withdrawals = $withdrawalsQuery
            ->orderBy('id', 'desc') // Use ID for better performance than created_at on large datasets
            ->paginate($perPage)
            ->appends($request->query()); // Preserve query parameters in pagination links

        // Optimize stats queries with single aggregation query for better performance
        $statsQuery = Withdrawal::selectRaw('
            ...
            SUM(CASE WHEN withdraw_type = "deposit" THEN 1 ELSE 0 END) as deposit_withdrawals,
            SUM(CASE WHEN withdraw_type = "wallet" THEN 1 ELSE 0 END) as wallet_withdrawals
        ')->first();
        ...
        $stats = [
            ...
            'deposit_withdrawals' => $statsQuery->deposit_withdrawals ?? 0,
            'wallet_withdrawals' => $statsQuery->wallet_withdrawals ?? 0,
        ];

        // Get available payment methods for filter dropdown
        $paymentMethods = \App\Models\WithdrawMethod::where('status', 1)
            ->orderBy('name')
            ->pluck('name', 'id')
            ->toArray();

        return view('admin.withdrawals.index', compact('pageTitle', 'withdrawals', 'stats', 'perPage', 'allowedPerPage', 'paymentMethods'));
    }
